...
export async function verifyChainHash(filePath) {
  try {
    ...
    const h = await api.sha256Hex(canonicalizeText(latest.text || ''));
    ...
    try {
      const v1 = await api.getTranscriptVersion(filePath, 1);
      if (!v1) return { ok: false, reason: 'missing-v1', got: h, expected };
      let text = canonicalizeText(v1.text || '');
      const edits = (await api.getTranscriptEdits(filePath)).sort((a, b) => (a.child_version - b.child_version));
      ...
      const h2 = await api.sha256Hex(text);
